...
//		pnl2.add(rb2);
	...
	public boolean checkPwd(String inNo, String inPwd) {
		...
		if(dbinfo.infoComparison(Integer.parseInt(inNo)) == false) {	// 해당 아이디가 없을 경우
		...
		// 해당 아이디가 있을 경우
		try {
			regNo = dbinfo.myInfo(Integer.parseInt(inNo), 2);	//	교번 학번 가져옴
			regPwd = dbinfo.myInfo(Integer.parseInt(inNo), 3); // 비번 가져옴
